...
def test_workspace_leases_refcount_manifest_layers() -> None:
    ...
    assert registry.leased_layers() == manifest.layers
...
def test_lease_head_layers_use_only_newest_layer_per_lease() -> None:
    ...
    assert registry.lease_head_layers() == (layer_2, layer_3)
    assert registry.leased_layers() == (layer_1, layer_2, layer_3)
