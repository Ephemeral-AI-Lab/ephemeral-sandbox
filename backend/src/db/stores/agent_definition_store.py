...
class AgentDefinitionStore:
    ...
    def get_by_name(self, name: str) -> AgentDefinitionRecord | None:
        """Look up an active agent definition by unique name."""
        with self._sf() as db:
            return (
    ...
    def get_by_id(self, record_id: str) -> AgentDefinitionRecord | None:
        """Look up an agent definition by primary key."""
        with self._sf() as db:
    ...
    def list_active(
        ...
        *,
        ...
        """Return active agent definitions, optionally filtered by tags."""
        with self._sf() as db:
            q = (
                db.query(AgentDefinitionRecord)
            ...
            if tags:
                # Filter records whose tags JSON array contains ALL requested tags
                for tag in tags:
    ...
    def update(self, name: str, updates: dict) -> AgentDefinitionRecord:
        """Apply partial updates and bump version. Uses optimistic locking."""
        with self._sf() as db:
            record = (
                db.query(AgentDefinitionRecord)
                .filter(
    ...
    def soft_delete(self, name: str) -> bool:
        """Mark an agent definition as inactive. Returns True if found."""
        with self._sf() as db:
            record = (
                db.query(AgentDefinitionRecord)
                .filter(
    ...
    def hard_delete(self, name: str) -> bool:
        """Permanently remove an agent definition. Returns True if found."""
        with self._sf() as db:
            record = (
                db.query(AgentDefinitionRecord)
                .filter(AgentDefinitionRecord.name == name)
                .first()
            )
            if record is None:
                return False
