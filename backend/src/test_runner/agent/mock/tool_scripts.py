    ...
    return PreparedToolScript(
        ...
        steps=(
            ToolScriptStep(
                "create-ledger-dir",
                exec_command_tool,
                {"cmd": f"mkdir -p {_PACKAGE_DIR}", "timeout": 60},
            ),
            ToolScriptStep(
...
def recursive_step_script(ctx: ScenarioContext) -> PreparedToolScript:
    ...
    steps: list[ToolScriptStep] = [
        ToolScriptStep(
            ...
            exec_command_tool,
            {"cmd": f"mkdir -p {_RECURSIVE_DIR}", "timeout": 60},
        ),
        ToolScriptStep(
    ...
    steps.append(
        ToolScriptStep(
            ...
            exec_command_tool,
            {
                "cmd": f"test -s {evidence_path} && ls -1 {_RECURSIVE_DIR}",
                "timeout": 60,
            },
        )
    )
    ...
    return PreparedToolScript(
        ...
        steps=(
            ToolScriptStep(
                "ensure-final-dir",
                exec_command_tool,
                {"cmd": f"mkdir -p {_ROOT}", "timeout": 60},
            ),
            ToolScriptStep(
                "write-final-stage",
                write_file_tool,
