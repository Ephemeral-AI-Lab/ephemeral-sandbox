...
- **The original Phase-1 integration stubs have been retired in Rust.** The
  ...
  are now Rust-native; the remaining risk is live, non-injected E2E coverage
  across the whole `root → delegate → terminal` path, not a Python fallback.
...
4. **Done:** the shipped binary seeds the root registry from `.eos-agents`.
   **Remaining proof:** run the broad non-injected `root → delegate → terminal`
   E2E before deleting `backend/src`.
